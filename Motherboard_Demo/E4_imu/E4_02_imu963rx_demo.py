...
# ------------------------------------------------------------------------------
#   构造接口 用于构建一个 IMU963RX 对象
#   IMU963RX_obj = IMU963RX(capture_div = 1, imu_type = IMU963RX.TYPE_RA, quar_rate = IMU963RX.RATE_DISABLE)
#       capture_div     采集分频    |   可选关键字参数 默认为 1 也就是每次都采集 代表多少次触发进行一次采集
#       imu_type        模块型号    |   可选关键字参数 IMU963RX.TYPE_x, x = (AUTO, RA) 默认 AUTO
#       quar_rate       硬解频率    |   可选关键字参数 IMU963RX.RATE_x, x = (DISABLE) 默认 DISABLE
#       return          返回内容    |   正常情况下返回对应 IMU963RX 的对象
# ------------------------------------------------------------------------------
imu = IMU963RX()
