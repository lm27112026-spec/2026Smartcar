...
//用于存储收到的热量
volatile uint16_t board_heat_limit = 0;
volatile uint16_t board_current_heat = 0;
...
static void DecodeStdIdData(hcan_t * CAN, CAN_RxHeaderTypeDef * rx_header, uint8_t rx_data[8])
{
    ...
    switch (type_id) {
		...
		case CAN_STD_ID_Referee: {
			// 把 rx_data 的8个字节拷贝到共用体中
            memcpy(&RECEIVE_CBC.referee_data.raw.data, rx_data, 8);
            // 给全局变量赋值
            board_heat_limit = RECEIVE_CBC.referee_data.raw.packed.heat_limit;
            board_current_heat = RECEIVE_CBC.referee_data.raw.packed.heat;
...
void get_shoot_heat_from_can(uint16_t *heat_limit, uint16_t *heat)
{
    if(heat_limit != NULL && heat != NULL)
    {
        *heat_limit = board_heat_limit;
        *heat = board_current_heat;
    }
}
