...
def demo():
    """
    yaw_hold.py 独立演示
    在 YAW_HOLD 模式下旋转到 90° 并保持，SW2 退出
    """
    import gc, time
    from machine import Pin
    from imu import IMU
    from motor import (stop_all, omni_drive_closed_loop,
                       get_encoder_speeds_filtered, reset_encoder_filter,
                       reset_wheel_pi)
    ...
    # ── 初始化 ──
    print("=" * 50)
    print("YawHolder Demo (借鉴 led yaw 控制模式)")
    print("=" * 50)

    imu = IMU(calibrate_on_init=True)
    imu.start()
    time.sleep_ms(200)
    imu.set_zero_reference()

    holder = YawHolder(imu)
    holder.set_target(90)  # 目标航向 90°
    ...
    stop_all()
    for _ in range(5):
        from motor import get_encoder_counts
    ...
    try:
        while True:
            ...
            # Yaw 保持计算
            wz = holder.compute(DT)

            # 电机控制
            if abs(wz) > 0.001:
                speeds = get_encoder_speeds_filtered(DT)
                omni_drive_closed_loop(0, 0, wz, speeds, DT)
            else:
                stop_all()
            ...
            if time.ticks_diff(now, last_print_ms) >= 300:
                ...
                err = holder.get_yaw_error()
                print("[{:04d}] yaw={:+6.1f}°  err={:+5.1f}°  wz={:+.3f}  aligned={}".format(
                    loop_cnt, yaw, err, wz, holder.is_aligned))
                led.toggle()
                last_print_ms = now
