...
【使用】
  from slave_motor import set_motor, omni_drive, get_encoder_speeds

【注意】此文件为从车专用，与 master 的 motor.py 引脚映射不同：
  - 从车电机引脚为 B26/C20/C24/C26 + C28-C31/D4-D7
  - 运动学使用 +wz（与 motor.py 一致）
"""
...
# 左前 (LF)      C3             C2                C20       M2
# 右前 (RF)      C1             C0                C24       M3
# 左后 (LB)      D14            D13           	  B26       M1
# 右后 (RB)      D16            D15               C26       M4
# ============================================================
...
# ============================================================
#  四、硬件初始化（PWM + 方向引脚）
#  这些代码在 import slave_motor 时自动执行一次
#  从车引脚映射（与 master motor.py 不同）：
#  MOTOR_LF = (pwm_2, pin_c30, pin_c31)  C20 + C30/C31
#  MOTOR_LB = (pwm_1, pin_c28, pin_c29)  B26 + C28/C29
#  MOTOR_RF = (pwm_3, pin_d4,  pin_d5)   C24 + D4/D5
#  MOTOR_RB = (pwm_4, pin_d6,  pin_d7)   C26 + D6/D7
# ============================================================
...
# 从车电机元组 (PWM, 方向A, 方向B) — 与 master motor.py 不同！
MOTOR_LF = (pwm_2, pin_c30, pin_c31)   # Left Front: C20 + C30/C31
MOTOR_LB = (pwm_1, pin_c28, pin_c29)   # Left Back:  B26 + C28/C29
MOTOR_RF = (pwm_3, pin_d4,  pin_d5)    # Right Front: C24 + D4/D5
MOTOR_RB = (pwm_4, pin_d6,  pin_d7)    # Right Back:  C26 + D6/D7
