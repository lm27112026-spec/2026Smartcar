...
user_file.write("%d\n%.1f\n%s\n"%(33,11.1,'Hello'))
user_file.write("BOARD TYPE : %s\n"%(BOARD_TYPE))
user_file.write("BOARD VERSION : %s\n"%(BOARD_VERSION))
# 将缓冲区数据写入到文件 清空缓冲区 相当于保存指令
user_file.flush()

# 不要在循环里去反复写入擦除文件 Flash 是有寿命的 禁不起这种折腾
# 不要在循环里去反复写入擦除文件 Flash 是有寿命的 禁不起这种折腾
# 不要在循环里去反复写入擦除文件 Flash 是有寿命的 禁不起这种折腾
...
print("将数据转换为各自类型")
print("int = %d"%(data1))
print("float = %.1f"%(data2))
print("str = %s"%(data3))

print("对转换的数据进行运算")
# 用整形数与浮点数相加得到浮点数
print("int + float = %.1f"%(data1 + data2))
# 强制转换结果浮点数为整形数
print("int(int + float) = %d"%(data1 + data2))
# 结果是数据计算
